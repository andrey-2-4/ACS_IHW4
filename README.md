...
mutex-ы используются для вывода в консоль (чтобы можно было нормально читать) и для методов продавцов (чтобы симулировать, что продавец обрабатывает запросы последовательно)
